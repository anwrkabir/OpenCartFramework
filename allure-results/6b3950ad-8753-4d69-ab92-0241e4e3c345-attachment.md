...
Received: false
    at addProductToCart (C:\OpenCartPlaywright\tests\EndToEndTest.spec.ts:161:94)
    at verifyShoppingCart (C:\OpenCartPlaywright\tests\EndToEndTest.spec.ts:204:67)
...
```ts
  ...
  159 |         
  160 |             //  4) Verify the product appears in the search results 
> 161 |             expect (await searchResultPage.isProductExistInSearchedList(config.productName)).toBeTruthy();
      |                                                                                              ^ Error: expect(received).toBeTruthy()
  162 |             
  163 |              //  5) Select the product in the searchProduct Page
  164 |             //If selectProduct may legitimately fail to find a product, and null is an expected outcome
  165 |             //Handled that null option  
  166 |             let productPage: ProductPage |null = await searchResultPage.selectProduct(config.productName);
  167 |         
  168 |             // Checking if the productPage is not null, then execute the following code
  169 |             if(!productPage){
  170 |                     console.log("Product not found seach results");
  171 |                     return null;
  172 |             }    
  173 |              //  6) Set the quantity
  174 |              await productPage.setProductQuantity(config.productQuantity);
  175 |         
  176 |             //  7) Add the product to the Cart
  177 |             await productPage.addProductToCart();
  178 |         
  179 |             //  8) Verify the success message
  180 |             expect(await productPage.isSuccessconfirmation).toBeTruthy();
  181 |             
  182 |             
  183 |            
  184 |             await productPage.clickBlackBoxCartField();
  185 |
  186 |           const shoppingCart =   await productPage.clickOnViewCartLink();
  187 |         
  188 |           return shoppingCart;
  189 |     }
  190 |
  191 |    async function verifyShoppingCart(page:Page){
  192 |
  193 |           /*      const shoppingCart :ShoppingCartPage|null =await  addProductToCart(page);
  194 |
  195 |                if(shoppingCart){
  196 |                const totalprice :string =await shoppingCart.getTotalPrice();
  197 |                
  198 |                console.log(`totalprice : ${totalprice}`);
  199 |                }
  200 |                else {
  201 |                 console.log("shopping cart is null.")
  202 |                } */
  203 |
  204 |                     const shoppingCart: ShoppingCartPage | null = await addProductToCart(page);
  205 |
  206 |     if (shoppingCart) {
  207 |         const totalPrice: string | null = await shoppingCart.getTotalPrice();
  208 |
  209 |         if (totalPrice !== null) {
  210 |             console.log(`Total price: ${totalPrice}`);
  211 |         } else {
  212 |             console.log("Total price is null or not found in the cart.");
  213 |         }
  214 |     } else {
  215 |         console.log("Shopping cart is null.");
  216 |     
  217 |    } 
  218 |
  219 |    }
