...
- Name: Log in test with CSV Data : Invalid login
- Location: C:\OpenCartPlaywright\tests\LoginDataDrivenCSV.spec.ts:15:9
...
Expected: " Your account has exceeded allowed number of login attempts. Please try again in 1 hour."
Received: " Warning: Your account has exceeded allowed number of login attempts. Please try again in 1 hour."
    at C:\OpenCartPlaywright\tests\LoginDataDrivenCSV.spec.ts:37:34
```
...
```ts
   ...
   7 |
   8 |
   9 | // Load csv test data from file( logindata.json)
  10 | const csvPath ="testdata/logindata.csv";
  11 | const csvTestData =  dataProvider.getTestDataFromCSV(csvPath);
  12 |
  13 | for(const data of csvTestData){
  14 |
  15 |     test(`Log in test with CSV Data : ${data.testName}`, async ({page})=>{
  16 |
...
> 37 |             expect(errorMessage).toBe(' Your account has exceeded allowed number of login attempts. Please try again in 1 hour.');
     |                                  ^ Error: expect(received).toBe(expected) // Object.is equality
